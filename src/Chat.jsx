...
/**
 * Chat.jsx (UI/UX polish)
 *
 * - Tightened input/menu layout
 * - Mobile adjustments (safe area, larger touch targets)
 * - Small in-app toast notification system (auto-dismiss)
 * - Escape key closes menu
 * - Keeps all colors / business logic intact
 */
...
export default function Chat() {
  ...
  const handleIncoming = (from, payloadOrText) => {
    ...
    if (
      ...
      // show a small toast for incoming messages when window not focused
      if (document.hidden || !document.hasFocus()) {
        const fromLabel = payloadOrText.fromName || payloadOrText.from;
        const snippet =
          typeof payloadOrText.text === "string"
            ? payloadOrText.text.slice(0, 80)
            : "[message]";
        pushToast(`${fromLabel}: ${snippet}`, "message");
      }
    ...
    if (typeof payloadOrText === "string") {
      ...
      maybeNotify(from, safeText);
      pushToast(`${from}: ${safeText}`, "message");
      return;
    }
  };
  ...
  // menu outside click handler + escape key to close
  useEffect(() => {
    const onDocClick = (e) => {
      if (!menuRef.current) return;
      if (!menuRef.current.contains(e.target)) setMenuOpen(false);
  ...
  const handleCreateHub = () => {
    ...
    setMenuOpen(false);
    pushToast("Hub created — share your ID to invite people", "success");
  };
  ...
  const handleJoinHub = async () => {
    ...
    setMenuOpen(false);
    pushToast(`Join requested: ${friendly}`, "info");
  };
  ...
  const handleConfirmLeave = () => {
    ...
    setConfirmLeaveOpen(false);
    pushToast("You left the hub", "info");
  };
  ...
  const renderMessage = (m, idx) => {
    ...
    return (
      <div
        onClick={() => handleTapMessage(m)}
        key={`${m.id ?? m.ts}-${idx}`}
        className={`p-2 rounded-2xl max-w-[52%] mb-2 cursor-pointer ${
          isMe ? "ml-auto bg-blue-500 text-white" : "bg-white/100  text-black"
        }`}
      >
        <div className="text-xs font-bold flex items-center">
  ...
  if (showNamePrompt) {
    return (
      <div className="h-screen flex items-center justify-center bg-gradient-to-br from-purple-200 to-purple-400 text-purple-600">
        <div className="bg-white/20 p-6 rounded-2xl text-center w-[90%] max-w-md">
          <h2 className="text-xl font-bold mb-4">Welcome to PeersHub</h2>
          <input
            value={username}
            onChange={(e) => setUsername(e.target.value)}
          ...
          <button
            onClick={() => {
              ...
              setUsername(username.trim());
              setShowNamePrompt(false);
              pushToast("Welcome! You can now create or join a hub", "success");
            }}
            className="px-4 py-3 rounded-lg bg-gradient-to-br from-purple-500 to-purple-700 text-white font-semibold w-full"
          >
            Continue 🚀
  ...
  // responsive container tweaks: narrower on larger screens, full width on mobile
  return (
    <>
      <div className="min-h-[90vh] md:h-[92vh] max-w-[420px] w-full mx-auto bg-gray-50 text-purple-600 p-4 md:p-6 flex flex-col rounded-3xl shadow-sm"
           style={{ boxSizing: "border-box", paddingBottom: "env(safe-area-inset-bottom, 16px)" }}>
        <header className="flex items-center justify-between mb-3">
          <div className="flex gap-2.5 items-baseline">
            <div className="text-sm text-blue-600">YourID</div>
            <div className="font-mono text-xs">{myId || "..."}</div>
            <div className="text-sm text-blue-600 ml-2">Name: {username}</div>
            <div className="text-xs text-purple-500 mt-1 ml-2">
              Auto-join: {joinedBootstrap || "none"}
            </div>
          </div>

          {/* three-dots menu */}
          <div className="relative" ref={menuRef} aria-haspopup="true">
            <button
              onClick={() => setMenuOpen((s) => !s)}
              className="p-2 rounded-full bg-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-300"
              aria-label="Menu"
              title="Menu"
            >
            ...
            <div
              className={`absolute right-0 mt-2 w-44 bg-white/10 backdrop-blur rounded-lg shadow-lg z-50 transform origin-top-right transition-all duration-150 ${
                menuOpen
                  ? "opacity-100 scale-100 pointer-events-auto"
                  : "opacity-0 scale-95 pointer-events-none"
              }`}
        ...
        <div className="w-full text-white h-0.5 bg-white/20 mb-2" />
        <main className="flex-1 overflow-auto mb-3 px-1">
          <div style={{ paddingBottom: 8 }}>
            {messages.length === 0 && (
              <div className="text-sm text-white/60">No messages yet</div>
            )}
        ...
        <footer className="mt-auto">
          {typingSummary()}
          <div className="mb-3 text-sm text-blue-600 flex items-center justify-between">
            <div>
              Connected peers:{" "}
              {connectedNames.length === 0 ? (
                <span className="text-red-500">none</span>
            ...
            <div className="text-xs text-gray-400" title={`${peers.length} peers`}>
              {peers.length} peers
            </div>
          </div>

          {replyTo && (
            <div className="mb-2 p-3 bg-white/10 text-gray-500 rounded-lg flex items-start justify-between">
              <div>
                Replying to <strong>{replyTo.from}</strong>:{" "}
                <span className="text-sm text-blue-400">{replyTo.text}</span>
              </div>
          ...
          <div className="relative w-full">
            ...
            <input
              ...
              onChange={(e) => setText(e.target.value)}
              placeholder="Type a message..."
              className="w-full p-3 pl-3 pr-10 bg-white/10 placeholder-blue-300 text-blue-500 font-mono rounded-3xl border-2 focus:outline-none focus:ring-2 focus:ring-blue-200"
              onKeyDown={(e) => {
                if (e.key === "Enter") send();
              }}
            ...
            <button
              onClick={send}
              className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-blue-200"
      ...
      <div className="fixed right-4 bottom-6 z-60 flex flex-col gap-3">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`min-w-[200px] max-w-sm px-4 py-2 rounded-lg shadow-lg text-sm ${
              t.type === "error"
