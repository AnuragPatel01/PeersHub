 ...
 * - stronger stop logic + per-peer retry/backoff + window control hooks
 *
 * New: Pre-join preview + join-request flow:
 * - requestPreview(bootstrapId) -> '__system_preview_response__' callback with anonymized peer list
 * - sendJoinRequest(bootstrapId, displayName, message) -> host receives '__system_join_request__'
 * - respondToJoinRequest(requestId, requesterPeerId, accept) -> requester receives '__system_join_response__'
 */
...
/* ---------- Pre-join preview and join-request APIs ---------- */

/**
 * requestPreview(bootstrapId)
 * - Ask the bootstrap (host) for an anonymized list of participants.
 * - UI should call this before joining to display a preview page.
 * - The host will reply with type "preview_response" routed back to the requester.
 */
export const requestPreview = (bootstrapId) => {
  if (!bootstrapId) throw new Error("bootstrapId required");
  ...
  // If we have a direct connection, send directly; otherwise broadcast with to so it routes.
  const conn = connections[bootstrapId];
  if (conn) sendToConn(conn, payload);
  else broadcastRaw(payload);

  return payload.id;
};

/**
 * sendJoinRequest(bootstrapId, displayName, message)
 * - Send a join request to the host (bootstrap).
 * - Host receives "__system_join_request__" event via onMessage.
 */
export const sendJoinRequest = (bootstrapId, displayName = "Visitor", message = "") => {
  if (!bootstrapId) throw new Error("bootstrapId required");
  if (!peer) throw new Error("peer not initialized");
  const reqId = `joinreq-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
  const payload = {
    type: "join_request",
    id: reqId,
    from: peer.id,
    name: displayName || peer.id,
    message: message || "",
    to: bootstrapId,
    ts: Date.now(),
  };
...
/**
 * respondToJoinRequest(requestId, requesterPeerId, accept)
 * - Host calls this to accept/decline. If accept == true, the host will also
 *   optionally (and automatically) call connectToPeer on the requester (the requester receives response).
 */
export const respondToJoinRequest = (requestId, requesterPeerId, accept = false) => {
  if (!requestId || !requesterPeerId) return;
  if (!peer) throw new Error("peer not initialized");
  const payload = {
    type: "join_response",
    id: `joinresp-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    reqId: requestId,
    from: peer.id,
    accept: !!accept,
    to: requesterPeerId,
  ...
  const conn = connections[requesterPeerId];
  if (conn) sendToConn(conn, payload);
  else broadcastRaw(payload);
};
...
const setupConnection = (
  ...
  conn.on("data", (raw) => {
    ...
    // FILE/preview/join handling added here

    // PREVIEW: requester asks bootstrap for anonymized peer list
    if (data.type === "preview_request") {
      // only respond if we are the bootstrap host (stored)
      try {
        const bootstrap = localStorage.getItem(LS_HUB_BOOTSTRAP);
        if (bootstrap && peer && peer.id === bootstrap) {
          // build anonymized list (User 1, User 2, ...). include host in list as 'Host' for clarity
          const anonymized = (peersList || []).map((pid, idx) => {
            const rawName = peerNames[pid] || "Anon";
            // keep a short anonymized label
            return { id: pid, anonName: `User ${idx + 1}` };
          });
          // include host as User 0 / Host
          const hostAnon = { id: peer.id, anonName: "Host" };
          const payload = {
            type: "preview_response",
            id: `previewresp-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,
            from: peer ? peer.id : null,
            to: data.from || conn.peer,
            peers: [hostAnon, ...anonymized],
          };
          // direct reply
          const targetConn = connections[data.from] || null;
          if (targetConn) sendToConn(targetConn, payload);
          else broadcastRaw(payload);
        }
      } catch (e) {
        console.warn("preview_request handler failed", e);
      }
      return;
    }

    // PREVIEW response (requester receives)
    if (data.type === "preview_response") {
      if (onMessage) {
        onMessage("__system_preview_response__", {
          id: data.id,
    ...
    // JOIN REQUEST: requester -> bootstrap (host)
    if (data.type === "join_request") {
      // host should receive this; forward to UI via onMessage
      if (onMessage) {
        onMessage("__system_join_request__", {
          id: data.id,
          from: data.from || conn.peer,
    ...
    // JOIN RESPONSE: host -> requester (accept/decline)
    if (data.type === "join_response") {
      const requester = data.to || data.from || conn.peer;
      if (onMessage) {
        onMessage("__system_join_response__", {
          id: data.id,
          reqId: data.reqId,
          from: data.from || conn.peer,
          accept: !!data.accept,
      ...
      // if accepted, auto-attempt to connect to the host (so user joins)
      try {
        if (data.accept && peer && data.from) {
          // attempt a direct connect using existing callbacks (onMessageGlobal, onPeerListUpdateGlobal)
          // This will respect your usual retry/backoff logic inside connectToPeer
          connectToPeer(data.from, onMessageGlobal, onPeerListUpdateGlobal, (peer && peer.id && peerNames[peer.id]) || "Anonymous");
        }
      } catch (e) {
        console.warn("auto connect after join_response failed", e);
      }
      return;
    }
    ...
    // routing: if 'to' present and not for me, ignore (common guard)
